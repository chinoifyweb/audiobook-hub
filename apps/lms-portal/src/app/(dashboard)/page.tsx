...
export default async function DashboardPage() {
  try {
    ...
    const [activeSemester, enrolledCoursesCount, pendingAssignments, upcomingTests, recentGrades, allGrades] =
      await Promise.all([
        ...
        prisma.semester.findFirst({ where: { isActive: true } }).then((sem) =>
          sem
            ? prisma.courseEnrollment.count({
                where: { studentId: studentProfile.id, semesterId: sem.id, status: "enrolled" },
              })
            : 0
        ).catch(() => 0),
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <div className="mt-4">
                <Link href="/grades" className="text-sm text-primary hover:underline">
                  View all grades
                </Link>
              </div>
            </CardContent>

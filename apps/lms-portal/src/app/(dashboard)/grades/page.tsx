  ...
  try {
    ...
    const [grades, semesters] = await Promise.all([
      ...
      prisma.semester.findMany({
        where: {
          grades: { some: { studentId: studentProfile.id, isReleased: true } },
        },
        include: { session: true },
        orderBy: [{ session: { startDate: "desc" } }, { number: "asc" }],
      }),
    ]);
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground">Academic performance and transcripts</p>
          </div>
          <Link
            href="/api/transcript"
        ...
        {semesters.length === 0 ? (
          <Card>
            <CardContent className="py-12 text-center">
              ...
              <p className="mt-4 text-muted-foreground">No grades have been released yet.</p>
            </CardContent>
          </Card>
        ) : (
                ...
                <CardContent>
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {semGrades.map((g) => {
                          const course = g.courseEnrollment.courseAssignment.course;
                          return (
                            <tr key={g.id} className="border-b last:border-0">
                              <td className="py-3 font-medium">{course.code}</td>
                              <td className="py-3">{course.title}</td>
                              <td className="py-3 text-center">{course.creditUnits}</td>
                              <td className="py-3 text-center">{Number(g.testScore).toFixed(0)}</td>
                              <td className="py-3 text-center">{Number(g.examScore).toFixed(0)}</td>
                              <td className="py-3 text-center">{Number(g.assignmentScore).toFixed(0)}</td>
                              <td className="py-3 text-center font-medium">{Number(g.totalScore).toFixed(0)}</td>
                              <td className="py-3 text-center">
                                <span className={`inline-flex items-center justify-center h-7 w-7 rounded-full text-xs font-bold ${getLetterGradeColor(g.letterGrade)}`}>
                                  {g.letterGrade || "-"}
                                </span>
                              </td>
                              <td className="py-3 text-center">{Number(g.gradePoint).toFixed(2)}</td>
                            </tr>
                          );
                        })}
                      </tbody>
                  ...
                  <div className="flex justify-between text-sm">
                    <span className="text-muted-foreground">Total Credits: {semCredits}</span>
                    <span className="font-medium">Semester GPA: {semGpa.toFixed(2)}</span>
                  </div>
                </CardContent>
              </Card>
            );
      ...
      <div className="space-y-6">
        ...
        <Card className="border-destructive">
          <CardContent className="py-8 text-center">
            ...
            <p className="text-muted-foreground">Failed to load grades. Please refresh the page.</p>
          </CardContent>
        </Card>
      </div>
    );
